...
def register_member(member_name):
    ...
    membership_fee = float(input("Enter the membership fee amount: "))

    # Insert new member into Members table
    cur.execute("INSERT INTO Members (Name, Password, FitnessGoal, HealthMetrics) VALUES (%s, %s, %s, %s) RETURNING MemberID",
                (member_name, password, fitness_goal, health_metrics))
...
def schedule_session():
    ...
    payment_status = process_membership_fee(member_id, membership_fee)
...
def update_class_schedule():
    ...
    if result is None:
        ...
        sql = f"INSERT INTO Classes (ClassID, TrainerID, ClassTime) VALUES ({class_id}, {trainer_id}, '{new_time}')"
        print("The class has been created successfully.")
    else:
        # If the class exists, update the class schedule
        sql = f"UPDATE Classes SET TrainerID={trainer_id}, ClassTime='{new_time}' WHERE ClassID={class_id}"
